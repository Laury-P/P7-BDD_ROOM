...
class DeleteExerciseUseCaseTest {
    ...
    fun `when repository returns success, use case should return success`() = runBlocking {
        ...
        coEvery{ exerciseRepository.deleteExercise(any()) } returns Result.success(true)

        // Act
        val result = deleteExerciseUseCase.execute(fakeExercise)
    ...
    fun `when repository returns failure, use case should return failure`() = runBlocking {
        ...
        coEvery{ exerciseRepository.deleteExercise(any()) } returns Result.failure(exception = Exception())
